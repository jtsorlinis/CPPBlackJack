...
void Table::hit() {
  ...
  if (m_verbose_) {
    std::cout << "Player " << m_players_[m_current_player_].m_player_num_ << " hits\n";
  }
}

void Table::stand() {
  if (m_verbose_ && m_players_[m_current_player_].m_value_ <= 21) {
    std::cout << "Player " << m_players_[m_current_player_].m_player_num_ << " stands\n";
    print();
  }
...
void Table::split() {
  ...
  m_players_.insert(m_players_.begin() + m_current_player_ + 1, std::move(split_player));
  m_players_[m_current_player_].evaluate();
  m_players_[m_current_player_+1].evaluate();
  if (m_verbose_) {
    std::cout << "Player " << m_players_[m_current_player_].m_player_num_ << " splits\n";
  }
}
  ...
  while (!m_players_[m_current_player_].m_is_done_) {
    ...
    if (m_players_[m_current_player_].m_hand_.size() < 5 &&
      ...
      if (split_card_val == 11) {
        ...
        action(get_action(m_players_[m_current_player_].m_value_, m_dealer_.up_card(),
                          &m_strat_soft_));
      } else {
        action(get_action(m_players_[m_current_player_].m_value_, m_dealer_.up_card(),
                          &m_strat_hard_));
