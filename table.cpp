...
Table::Table(int numPlayers, int numOfDecks, int betSize, int minCards,
             int verbose)
    : mCardPile(CardPile(numOfDecks)) {
  mVerbose = verbose;
  mBetSize = betSize;
...
void Table::selectBet(Player* player) {
  if (mTrueCount >= 2) {
    player->mInitialBet = static_cast<int>(mBetSize * (mTrueCount - 1) * 1.25);
  }
}
...
void Table::startRound() {
  ...
  if (mVerbose > 0) {
    ...
    std::cout << "Running count is: " << mRunningcount
              << "\tTrue count is: " << mTrueCount << "\n";
  }
  getNewcards();
  preDeal();
  dealRound();
...
void Table::getNewcards() {
  if (mCardPile.mCards.size() < mMinCards) {
    ...
    mRunningcount = 0;
    if (mVerbose > 0) {
      std::cout << "Got " << mNumOfdecks
                << " new decks as number of cards is below " << mMinCards
                << "\n";
    }
  }
}
...
void Table::clear() {
  mPlayers.erase(std::remove_if(mPlayers.begin(), mPlayers.end(),
                                [](auto& player) { return player.mSplitFrom; }),
                 mPlayers.end());
  for (auto& player : mPlayers) {
    player.resetHand();
  }
...
void Table::updatecount() {
  if (mCardPile.mCards.size() > 51) {
    mTrueCount = mRunningcount / static_cast<int>(mCardPile.mCards.size() / 52);
  }
}
...
void Table::autoPlay() {
  while (!mCurrentPlayer->mIsDone) {
    ...
    if (mCurrentPlayer->mHand.size() == 1) {
      if (mVerbose > 0) {
        std::cout << "Player " << mCurrentPlayer->mPlayerNum
    ...
    if (mCurrentPlayer->mHand.size() < 5 && mCurrentPlayer->mValue < 21) {
      int splitCardVal = mCurrentPlayer->canSplit();
      if (splitCardVal == 11) {
        splitAces();
      } else if (splitCardVal != 0 &&
                 (splitCardVal != 5 && splitCardVal != 10)) {
        action(getAction(splitCardVal, mDealer.upCard(), &mStratSplit));
      } else if (mCurrentPlayer->mIsSoft) {
        action(
            getAction(mCurrentPlayer->mValue, mDealer.upCard(), &mStratSoft));
      } else {
        action(
            getAction(mCurrentPlayer->mValue, mDealer.upCard(), &mStratHard));
      }
    } else {
      stand();
    }
...
void Table::checkPlayerNatural() {
  for (auto& player : mPlayers) {
    if (player.mValue == 21 && player.mHand.size() == 2 &&
        player.mSplitFrom == nullptr) {
...
void Table::finishRound() {
  ...
  for (auto& player : mPlayers) {
    if (player.mHasNatural) {
      player.win(1.5);
      if (mVerbose > 0) {
        std::cout << "Player " << player.mPlayerNum << " Wins "
                  << (1.5 * player.mBetMult * player.mInitialBet)
                  << " with a natural 21\n";
      }
    } else if (player.mValue > 21) {
      player.lose();
      if (mVerbose > 0) {
        std::cout << "Player " << player.mPlayerNum << " Busts and Loses "
                  << (player.mBetMult * static_cast<float>(player.mInitialBet))
                  << "\n";
      }
      ...
      player.lose();
      if (mVerbose > 0) {
        std::cout << "Player " << player.mPlayerNum << " Loses "
                  << (player.mBetMult * static_cast<float>(player.mInitialBet))
                  << "\n";
      }
    }
  ...
  if (mVerbose > 0) {
    for (auto& player : mPlayers) {
      if (player.mSplitFrom == nullptr) {
        std::cout << "Player " << player.mPlayerNum
                  << " Earnings: " << player.mEarnings << "\n";
      }
    }
    std::cout << "\n";
  }
